...
def test_show_variables(runner, tmp_path, invoke):
    ...
    with PFBWriter(str(output_path)) as writer:
        ...
        writer.set_variables(["submitter_id", "id"])
    ...
    result = invoke("show", "-i", str(output_path), "variables")

    assert result.exit_code == 0, result.output
    assert '"submitter_id"' in result.output
    assert '"id"' in result.output


def test_show_aggregations(runner, tmp_path, invoke):
    output_path = tmp_path / "out.avro"
