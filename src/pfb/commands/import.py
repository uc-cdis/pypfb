...
def run(ctx):
    ctx.ensure_object(dict)
    try:
        auth = Gen3Auth(ctx.obj["data_commons"], refresh_file = ctx.obj["credentials_file"])
...
def dry(ctx):
    ...
    try:
        ...
        if auth_request.status_code != 200:
            click.secho("There was an error with getting an access token. Are your api credentials valid and are you using the correct commons?", fg="red", bold=True, err=True)
            token_flag = False
        else:
        ...
        if not isinstance(signed_url, dict):
            click.secho("Could not get a presigned url check your GUID", fg="red", bold=True, err=True)
            guid_flag = False
        ...
        if sower_test.status_code != 200:
            click.secho("There was an error with sower access. Check sower permissions!", fg="red", bold=True, err=True)
            sower_flag = False
        else:
            click.secho("Sower permissions OK!\n", fg="green", err=True, bold=True)

        if not guid_flag or not sower_flag:
            click.secho("Something is wrong. Check GUID or Sower permissions", fg="red", bold=True, err=True)
